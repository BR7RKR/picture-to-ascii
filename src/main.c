...
static int float_parse(const char *restrict str, float *restrict out_value);
...
int main(int argc, char *restrict argv[]){
    ...
    if (path_to_save_img) {
        ...
        if (result != ASCII_IMG_OK) {
            log_error("failed to save ascii image as an image");
        } else {
            log_info("ascii image was saved successfully as an image");
        }
    }
...
static void print_help(void){
    ...
    puts("  -if, --image-file <path>  Save output to file at specified path as an image");
